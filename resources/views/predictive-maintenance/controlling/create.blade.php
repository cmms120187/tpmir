...
<div class="w-full p-4 sm:p-6 lg:p-8">
    <div class="w-full mx-auto max-w-6xl">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Create Predictive Maintenance Execution</h1>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="hidden" name="machine_id" id="machine_id_hidden" value="{{ old('machine_id', $selectedMachineId ?? '') }}">
                        @error('machine_id')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadMachinesByType(typeId) {
        return new Promise((resolve, reject) => {
            ...
            const url = `{{ route('predictive-maintenance.controlling.get-machines-by-type') }}?type_id=${typeId}`;
    ...
    // Calculate measurement status based on standard
    function calculateMeasurementStatus(measuredValue, standardMin, standardMax, standardTarget) {
        ...
        if (standardMin !== null && standardMax !== null) {
            if (value < standardMin || value > standardMax) {
                ...
                const range = standardMax - standardMin;
                const tolerance = range * 0.1; // 10% tolerance
                if (Math.abs(value - standardTarget) > tolerance) {
                    return 'warning';
                }
            }
            return 'normal';
    ...
    function showMaintenancePoints() {
        ...
        const url = `{{ route('predictive-maintenance.controlling.get-maintenance-points-by-machine-and-date') }}?machine_id=${machineId}&scheduled_date=${scheduledDate}`;
            ...
            .then(data => {
                ...
                if (data.maintenance_points && data.maintenance_points.length > 0) {
                    data.maintenance_points.forEach((point, index) => {
                        ...
                        const currentMeasuredValue = point.measured_value || '';
                        const currentMeasurementStatus = point.measurement_status || '';
                        ...
                        // Standard info
                        const standardInfo = point.standard_name && point.standard_name !== '-'
                            ? `<div class="mt-2 p-2 bg-blue-50 rounded border border-blue-200">
                                ...
                                <p class="text-xs text-blue-700">Range: ${point.standard_min !== null ? point.standard_min : '-'} - ${point.standard_max !== null ? point.standard_max : '-'} ${point.standard_unit || ''}</p>
                                ${point.standard_target !== null ? `<p class="text-xs text-blue-700">Target: ${point.standard_target} ${point.standard_unit || ''}</p>` : ''}
                            </div>`
                            : '';
                        ...
                        if (currentMeasurementStatus) {
                            ...
                            measurementStatusBadge = `<span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold ${statusColors[currentMeasurementStatus] || 'bg-gray-100 text-gray-800'} ml-2">${statusText[currentMeasurementStatus] || currentMeasurementStatus}</span>`;
                        }
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1 ${point.photo ? 'ml-0' : ''}">
                                    ...
                                    <div class="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-2">Measured Value <span class="text-gray-500 text-xs">(${point.standard_unit || ''})</span></label>
                                            <input type="number"
                                                   step="0.01"
                                                   name="executions[${index}][measured_value]"
                                                   ...
                                                   data-unit="${point.standard_unit || ''}"
                                                   value="${currentMeasuredValue}"
                                                   placeholder="Masukkan nilai pengukuran">
                                            <p class="text-xs text-gray-500 mt-1">Range: ${point.standard_min !== null ? point.standard_min : '-'} - ${point.standard_max !== null ? point.standard_max : '-'}</p>
                                        </div>
                                        <div>
                        ...
                        // Add event listener for measured value input to show status
                        const measuredValueInput = pointDiv.querySelector('.measured-value-input');
                        if (measuredValueInput) {
                            measuredValueInput.addEventListener('input', function() {
                                const value = parseFloat(this.value);
                                const min = parseFloat(this.dataset.min);
                                const max = parseFloat(this.dataset.max);
                                const target = parseFloat(this.dataset.target);

                                // Update measurement status display (visual feedback only, server will calculate)
                                const statusBadge = pointDiv.querySelector('.measurement-status-badge');
                                if (statusBadge && !isNaN(value)) {
                                    const status = calculateMeasurementStatus(value, min, max, target);
                                    if (status) {
                                        const statusColors = {
                                            'normal': 'bg-green-100 text-green-800',
                                            'warning': 'bg-yellow-100 text-yellow-800',
                                        ...
                                        statusBadge.className = `inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold ${statusColors[status]} ml-2`;
