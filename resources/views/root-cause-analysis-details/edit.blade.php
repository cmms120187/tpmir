...
<div class="w-full p-4 sm:p-6 lg:p-8">
    <div class="w-full max-w-6xl mx-auto">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Edit Root Cause Analysis</h1>
            <a href="{{ route('root-cause-analysis-details.show', $rca->id) }}" class="text-gray-600 hover:text-gray-800">
        ...
        <form action="{{ route('root-cause-analysis-details.update', $rca->id) }}" method="POST" class="bg-white rounded-lg shadow p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $rca->title) }}" required
                    ...
                    <div>
                        ...
                        <select name="analysis_method" id="analysis_method" required
                            ...
                            <option value="5_whys" {{ old('analysis_method', $rca->analysis_method) == '5_whys' ? 'selected' : '' }}>5 Whys</option>
                            <option value="fishbone" {{ old('analysis_method', $rca->analysis_method) == 'fishbone' ? 'selected' : '' }}>Fishbone (Ishikawa)</option>
                            <option value="fault_tree" {{ old('analysis_method', $rca->analysis_method) == 'fault_tree' ? 'selected' : '' }}>Fault Tree Analysis</option>
                            <option value="pareto" {{ old('analysis_method', $rca->analysis_method) == 'pareto' ? 'selected' : '' }}>Pareto Analysis</option>
                            <option value="combined" {{ old('analysis_method', $rca->analysis_method) == 'combined' ? 'selected' : '' }}>Kombinasi</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" name="analysis_date" id="analysis_date" value="{{ old('analysis_date', $rca->analysis_date->format('Y-m-d')) }}" required
                    ...
                    <div>
                        ...
                        <select name="downtime_id" id="downtime_id"
                            ...
                            @foreach($downtimes as $downtime)
                                <option value="{{ $downtime->id }}" {{ old('downtime_id', $rca->downtime_id) == $downtime->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="machine_id" id="machine_id"
                            ...
                            @foreach($machines as $machine)
                                <option value="{{ $machine->id }}" {{ old('machine_id', $rca->machine_id) == $machine->id ? 'selected' : '' }}>
                                    {{ $machine->idMachine }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                            ...
                            <option value="in_progress" {{ old('status', $rca->status) == 'in_progress' ? 'selected' : '' }}>Sedang Dikerjakan</option>
                            <option value="completed" {{ old('status', $rca->status) == 'completed' ? 'selected' : '' }}>Selesai</option>
                            <option value="reviewed" {{ old('status', $rca->status) == 'reviewed' ? 'selected' : '' }}>Sudah Direview</option>
                        </select>
                        @error('status')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
            ...
            <div class="mb-6">
                ...
                <textarea name="problem_description" id="problem_description" rows="4" required
                          class="w-full border rounded px-3 py-2 @error('problem_description') border-red-500 @enderror">{{ old('problem_description', $rca->problem_description) }}</textarea>
            ...
            <div id="five_whys_form" class="mb-6">
                ...
                <div id="five_whys_container" class="space-y-4">
                    @php
                        $existingWhys = $rca->fiveWhys->keyBy('why_level');
                    @endphp
                    @for($i = 1; $i <= 5; $i++)
                    <div class="border rounded-lg p-4 bg-gray-50 five-why-item">
                        ...
                        <input type="hidden" name="five_whys[{{ $i-1 }}][why_level]" value="{{ $i }}">
                        <div class="mb-3">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Pertanyaan</label>
                            <input type="text" name="five_whys[{{ $i-1 }}][question]"
                                   value="{{ old("five_whys.{$i-1}.question", $existingWhys->get($i)?->question ?? ($i == 1 ? 'Mengapa masalah ini terjadi?' : '')) }}"
                                   placeholder="Mengapa...?"
                                   class="w-full border rounded px-3 py-2">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Jawaban</label>
                            <textarea name="five_whys[{{ $i-1 }}][answer]" rows="2"
                                      placeholder="Jawaban untuk Why #{{ $i }}"
                                      class="w-full border rounded px-3 py-2">{{ old("five_whys.{$i-1}.answer", $existingWhys->get($i)?->answer ?? '') }}</textarea>
                        </div>
                    </div>
                    @endfor
                </div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <textarea name="root_cause" id="root_cause" rows="3"
                                  class="w-full border rounded px-3 py-2 @error('root_cause') border-red-500 @enderror">{{ old('root_cause', $rca->root_cause) }}</textarea>
                        @error('root_cause')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="corrective_action" id="corrective_action" rows="3"
                                  class="w-full border rounded px-3 py-2 @error('corrective_action') border-red-500 @enderror">{{ old('corrective_action', $rca->corrective_action) }}</textarea>
                    ...
                    <div>
                        ...
                        <textarea name="preventive_action" id="preventive_action" rows="3"
                                  class="w-full border rounded px-3 py-2 @error('preventive_action') border-red-500 @enderror">{{ old('preventive_action', $rca->preventive_action) }}</textarea>
            ...
            <div class="flex justify-end gap-3">
                <a href="{{ route('root-cause-analysis-details.show', $rca->id) }}" class="px-4 py-2 border border-gray-300 rounded text-gray-700 hover:bg-gray-50">
                    Cancel
                </a>
