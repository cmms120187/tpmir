...
<div class="w-full p-4 sm:p-6 lg:p-8">
    <div class="w-full mx-auto">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Preventive Maintenance - Updating</h1>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-center">
                                    ...
                                    @endphp
                                    @if($firstExecution)
                                        <a href="{{ route('preventive-maintenance.updating.edit', $firstExecution->id) }}" 
        ...
        <div class="bg-white rounded-lg shadow p-4 sm:p-6 mt-6">
            ...
            @if($completedJadwalCollection->count() > 0)
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($completedJadwalCollection as $jadwal)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3 text-sm">
                                    <div class="font-semibold text-gray-900">{{ $jadwal['machine']->idMachine ?? '-' }}</div>
                                    <div class="text-gray-500">{{ $jadwal['machine']->machineType->name ?? '-' }}</div>
...
function showMaintenancePoints(machineId, scheduledDate) {
    ...
    const url = `{{ route('preventive-maintenance.updating.get-maintenance-points-by-machine-and-date') }}?machine_id=${machineId}&scheduled_date=${scheduledDate}`;
            ...
            data.maintenance_points.forEach((point, index) => {
                ...
                html += `
                    <div class="border rounded p-4 bg-white">
                        ...
                        ${point.execution_id ? `
                            <div class="mt-3">
                                <a href="{{ url('preventive-maintenance/updating') }}/${point.execution_id}/edit" 
