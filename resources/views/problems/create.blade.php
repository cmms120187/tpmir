...
<div class="w-full p-4 sm:p-6 lg:p-8">
    <div class="w-full mx-auto">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Create Problem</h1>
            ...
            <div class="bg-white rounded-lg shadow p-6 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="problem_header" id="problem_header" value="{{ old('problem_header') }}" placeholder="e.g., ELECTRICAL, MOTOR, MECHANICAL" class="w-full border rounded px-3 py-2 @error('problem_header') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="e.g., MESIN TIDAK DAPAT BEROPERASI" class="w-full border rounded px-3 py-2 @error('name') border-red-500 @enderror" required>
                    @error('name')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="problem_mm" id="problem_mm" class="w-full border rounded px-3 py-2 @error('problem_mm') border-red-500 @enderror">
                        ...
                        <option value="OTHER" {{ old('problem_mm') == 'OTHER' ? 'selected' : '' }}>OTHER</option>
                        <option value="MACHINE BREAKDOWN" {{ old('problem_mm') == 'MACHINE BREAKDOWN' ? 'selected' : '' }}>MACHINE BREAKDOWN</option>
                        <option value="CONVEYOR BELT CHANGEOVER" {{ old('problem_mm') == 'CONVEYOR BELT CHANGEOVER' ? 'selected' : '' }}>CONVEYOR BELT CHANGEOVER</option>
                        <option value="POWER SUPPLY PROBLEM" {{ old('problem_mm') == 'POWER SUPPLY PROBLEM' ? 'selected' : '' }}>POWER SUPPLY PROBLEM</option>
                    </select>
                    @error('problem_mm')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
