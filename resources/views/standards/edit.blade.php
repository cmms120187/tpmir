...
<div class="w-full p-4 sm:p-6 lg:p-8">
    <div class="w-full mx-auto max-w-4xl">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Edit Standard</h1>
        ...
        <form action="{{ route('standards.update', $standard->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $standard->name) }}" class="w-full border rounded px-3 py-2" required>
                        </div>
                        
                        <div>
                            <label for="unit" class="block text-sm font-medium text-gray-700 mb-2">Unit</label>
                            <input type="text" name="unit" id="unit" value="{{ old('unit', $standard->unit) }}" placeholder="°C, bar, mm/s, etc" class="w-full border rounded px-3 py-2">
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="reference_type" id="reference_type" class="w-full border rounded px-3 py-2">
                                ...
                                <option value="ISO" {{ old('reference_type', $standard->reference_type) == 'ISO' ? 'selected' : '' }}>ISO</option>
                                <option value="Factory Standard" {{ old('reference_type', $standard->reference_type) == 'Factory Standard' ? 'selected' : '' }}>Factory Standard</option>
                                <option value="Custom" {{ old('reference_type', $standard->reference_type) == 'Custom' ? 'selected' : '' }}>Custom</option>
                                <option value="Industry Standard" {{ old('reference_type', $standard->reference_type) == 'Industry Standard' ? 'selected' : '' }}>Industry Standard</option>
                            </select>
                        </div>
                        
                        <div>
                            <label for="reference_code" class="block text-sm font-medium text-gray-700 mb-2">Reference Code</label>
                            <input type="text" name="reference_code" id="reference_code" value="{{ old('reference_code', $standard->reference_code) }}" placeholder="ISO 9001, FS-001, etc" class="w-full border rounded px-3 py-2">
                        </div>
                        
                        <div class="md:col-span-2">
                            <label for="reference_name" class="block text-sm font-medium text-gray-700 mb-2">Reference Name</label>
                            <input type="text" name="reference_name" id="reference_name" value="{{ old('reference_name', $standard->reference_name) }}" placeholder="ISO 9001:2015 Quality Management" class="w-full border rounded px-3 py-2">
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="number" step="0.0001" name="min_value" id="min_value" value="{{ old('min_value', $standard->min_value) }}" class="w-full border rounded px-3 py-2">
                        ...
                        <div>
                            ...
                            <input type="number" step="0.0001" name="max_value" id="max_value" value="{{ old('max_value', $standard->max_value) }}" class="w-full border rounded px-3 py-2">
                            <p class="text-xs text-gray-500 mt-1">Akan diisi otomatis dari variants jika ada</p>
                        </div>
                        
                        <div>
                            <label for="target_value" class="block text-sm font-medium text-gray-700 mb-2">Target Value</label>
                            <input type="number" step="0.0001" name="target_value" id="target_value" value="{{ old('target_value', $standard->target_value) }}" class="w-full border rounded px-3 py-2">
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div>
                        ...
                        <div class="border rounded px-3 py-2 max-h-48 overflow-y-auto">
                            <div class="space-y-2">
                                @php
                                    $selectedMachineTypeIds = old('machine_type_ids', $standard->machineTypes->pluck('id')->toArray());
                                @endphp
                                @foreach($machineTypes as $machineType)
                                    <label class="flex items-center">
                                        <input type="checkbox" name="machine_type_ids[]" value="{{ $machineType->id }}" 
                                               {{ in_array($machineType->id, $selectedMachineTypeIds) ? 'checked' : '' }}
                    ...
                    <div>
                        ...
                        <select name="status" id="status" class="w-full border rounded px-3 py-2" required>
                            <option value="active" {{ old('status', $standard->status) == 'active' ? 'selected' : '' }}>Active</option>
                            <option value="inactive" {{ old('status', $standard->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                        </select>
                    </div>
                    
                    <div class="mt-4">
                        <label for="description" class="block text-sm font-medium text-gray-700 mb-2">Description</label>
                        <textarea name="description" id="description" rows="3" class="w-full border rounded px-3 py-2">{{ old('description', $standard->description) }}</textarea>
                    </div>
                    
                    <div class="mt-4">
                        <label for="keterangan" class="block text-sm font-medium text-gray-700 mb-2">Keterangan</label>
                        <textarea name="keterangan" id="keterangan" rows="3" placeholder="sample" class="w-full border rounded px-3 py-2">{{ old('keterangan', $standard->keterangan ?? 'sample') }}</textarea>
                    ...
                    <div class="mt-4">
                        ...
                        <!-- Photo yang Sudah Terpasang -->
                        @if(($standard->photos && $standard->photos->count() > 0) || $standard->photo)
                            <div class="mb-4">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <p class="text-xs text-gray-500">Format: JPEG, PNG, JPG, GIF (Max: 2MB). Kosongkan jika tidak ingin menambah photo baru.</p>
...
<script>
    ...
    function addVariantRow(variant = null) {
        ...
        row.innerHTML = `
            ...
            ${variantId ? `<input type="hidden" name="variants[${variantIndex}][id]" value="${variantId}">` : ''}
        `;
