...
<div class="w-full p-4 sm:p-6 lg:p-8">
    <div class="max-w-4xl mx-auto bg-white rounded-lg shadow p-6 sm:p-8">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-800 mb-2">Edit Machine ERP</h1>
            <p class="text-sm text-gray-600">Update machine ERP information</p>
        </div>
        ...
        <form action="{{ route('machine-erp.update', $machineErp->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" 
                       ...
                       value="{{ old('idMachine', $machineErp->idMachine) }}" 
                       required
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" 
                           ...
                           value="{{ old('plant_name', $machineErp->plant_name) }}" 
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           value="{{ old('process_name', $machineErp->process_name) }}" 
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           value="{{ old('line_name', $machineErp->line_name) }}" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" 
                           ...
                           value="{{ old('room_name', $machineErp->room_name) }}" 
                ...
                <select id="model_select" 
                    ...
                    @foreach($models as $model)
                        <option value="{{ $model->id }}" 
                                {{ old('model_select', $machineErp->model_name && $machineErp->model_name == $model->name ? $model->id : '') == $model->id ? 'selected' : '' }}>
                            {{ $model->name }}
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                    ...
                    <input type="hidden" name="group_id" id="group_id" value="{{ old('group_id', $machineErp->machineType && $machineErp->machineType->group_id ? $machineErp->machineType->group_id : '') }}">
                    <input type="text" 
                           id="group_display" 
                           class="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-100 cursor-not-allowed" 
                           readonly
                           value="{{ old('group_display', $machineErp->machineType && $machineErp->machineType->groupRelation ? $machineErp->machineType->groupRelation->name : '') }}"
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           id="type_name" 
                           class="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-100 cursor-not-allowed" 
                           value="{{ old('type_name', $machineErp->type_name) }}" 
                           readonly
                           placeholder="Akan terisi otomatis dari Machine Type">
                    @error('type_name')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           id="brand_name" 
                           class="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-100 cursor-not-allowed" 
                           value="{{ old('brand_name', $machineErp->brand_name) }}" 
                           readonly
                           placeholder="Akan terisi otomatis dari Machine Type">
                    @error('brand_name')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" 
                           ...
                           id="model_name" 
                           class="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-100 cursor-not-allowed" 
                           value="{{ old('model_name', $machineErp->model_name) }}" 
                           readonly
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           value="{{ old('serial_number', $machineErp->serial_number) }}" 
                ...
                <div>
                    ...
                    <input type="number" 
                           ...
                           value="{{ old('tahun_production', $machineErp->tahun_production) }}" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" 
                           ...
                           value="{{ old('no_document', $machineErp->no_document) }}" 
                ...
                <div>
                    ...
                    @if($machineErp->photo)
                        <div class="mb-2">
                            ...
                            <img src="{{ asset('storage/' . $machineErp->photo) }}" alt="Current Photo" class="max-w-xs max-h-48 object-cover rounded border">
                        </div>
                    @elseif($machineErp->machineType && $machineErp->machineType->photo)
                        <div class="mb-2">
                            <p class="text-sm text-gray-600 mb-2">Using Machine Type Default Photo:</p>
                            <img src="{{ asset('storage/' . $machineErp->machineType->photo) }}" alt="Default Photo" class="max-w-xs max-h-48 object-cover rounded border">
                        </div>
                    @endif
                    <input type="file" 
                           name="photo" 
                    ...
                    @error('photo')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-xs text-gray-500 mt-1">Format: JPEG, PNG, JPG, GIF (Max: 5MB). Photo ini spesifik untuk model mesin ini. Kosongkan jika tidak ingin mengubah photo.</p>
            ...
            <div class="flex items-center gap-3 pt-4">
                ...
                <a href="{{ route('machine-erp.index', isset($page) ? ['page' => $page] : []) }}" class="text-gray-600 hover:text-gray-800 hover:underline transition">Cancel</a>
            </div>
        </form>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Set selected option based on current room_name if exists
    if (roomNameInput.value) {
        const options = roomErpSelect.options;
        for (let i = 0; i < options.length; i++) {
            if (options[i].dataset.room === roomNameInput.value) {
                // Check if plant, process, and line also match
                if (options[i].dataset.plant === plantNameInput.value &&
                    options[i].dataset.process === processNameInput.value &&
                    options[i].dataset.line === lineNameInput.value) {
                    roomErpSelect.value = options[i].value;
                    break;
                }
            }
        }
    ...
    roomErpSelect.addEventListener('change', function() {
        ...
        if (selectedOption.value) {
            // Auto-fill fields from selected room ERP
            plantNameInput.value = selectedOption.dataset.plant || '';
    ...
    // Current model name from database
    const currentModelName = '{{ old("model_name", $machineErp->model_name) }}';
    ...
    if (modelSelect) {
        modelSelect.addEventListener('change', function() {
            ...
            if (selectedModel) {
                ...
                // Fill Model Name
                modelNameSelect.value = selectedModel.name || '';
                ...
                if (selectedModel.group_id) {
                    ...
                    const selectedGroup = groupsData.find(g => g.id === selectedModel.group_id);
        ...
        // Trigger on page load if model is already selected
        if (modelSelect && currentModelName) {
            // Try to find matching model by name
            const matchingModel = modelsData.find(m => m.name === currentModelName);
            if (matchingModel) {
                modelSelect.value = matchingModel.id;
