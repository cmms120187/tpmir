...
<div class="w-full p-4 sm:p-6 lg:p-8 bg-gradient-to-br from-gray-50 via-blue-50 to-purple-50 min-h-screen">
    <div class="w-full mx-auto">
        ...
        <div class="mb-8 animate-fade-in">
            <h1 class="text-5xl font-bold bg-gradient-to-r from-purple-600 via-blue-600 to-indigo-600 bg-clip-text text-transparent mb-2">
                Dashboard
            </h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
            ...
            <div class="stat-card stat-card-1 rounded-xl shadow-xl p-6 text-white animate-fade-in-up delay-100 hover:shadow-2xl">
                ...
                <div>
                    <p class="text-sm font-medium text-white/80 mb-1">Total Breakdowns</p>
                    <p class="text-4xl font-bold mt-2 animate-pulse-slow">{{ number_format($monthDowntimeCount) }}</p>
            ...
            <div class="stat-card stat-card-2 rounded-xl shadow-xl p-6 text-white animate-fade-in-up delay-200 hover:shadow-2xl">
                ...
                <div>
                    <p class="text-sm font-medium text-white/80 mb-1">Total Duration</p>
                    <p class="text-4xl font-bold mt-2 animate-pulse-slow">{{ number_format($monthDowntime, 0) }}</p>
            ...
            <div class="stat-card stat-card-3 rounded-xl shadow-xl p-6 text-white animate-fade-in-up delay-300 hover:shadow-2xl">
                ...
                <div>
                    ...
                    <p class="text-4xl font-bold mt-2 animate-pulse-slow">{{ number_format($avgDowntimeDuration, 1) }}</p>
                    <p class="text-xs text-white/70 mt-2">minutes</p>
                </div>
            </div>
            ...
            <div class="stat-card stat-card-4 rounded-xl shadow-xl p-6 text-white animate-fade-in-up delay-400 hover:shadow-2xl">
                ...
                <div>
                    ...
                    <p class="text-4xl font-bold mt-2 animate-pulse-slow">{{ number_format($avgDowntimePerDay, 1) }}</p>
            ...
            <div class="stat-card stat-card-5 rounded-xl shadow-xl p-6 text-white animate-fade-in-up delay-500 hover:shadow-2xl">
                ...
                <div>
                    ...
                    @if($mostProblematicMachine)
                        <p class="text-2xl font-bold mt-2 animate-pulse-slow" title="{{ $mostProblematicMachine->idMachine }}">{{ strlen($mostProblematicMachine->idMachine) > 12 ? substr($mostProblematicMachine->idMachine, 0, 12) . '...' : $mostProblematicMachine->idMachine }}</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div class="chart-card rounded-xl shadow-lg p-6 aspect-square flex flex-col animate-fade-in-up delay-200">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('downtime_erp.index') }}" class="text-xs text-blue-600 hover:text-blue-800 font-semibold">View →</a>
                </div>
            ...
            <div class="info-card rounded-xl shadow-lg p-6 animate-fade-in-up delay-500">
                ...
                <div class="space-y-3">
                    ...
                    <div class="p-4 bg-gradient-to-r from-red-50 to-orange-50 rounded-lg">
                        ...
                        <p class="text-2xl font-bold text-red-600">{{ number_format($longestDowntime->duration ?? 0, 1) }} <span class="text-sm">min</span></p>
                    </div>
                    <div class="p-4 bg-gradient-to-r from-red-50 to-orange-50 rounded-lg">
            ...
            <div class="info-card rounded-xl shadow-lg p-6 animate-fade-in-up delay-600">
                ...
                <div class="space-y-3">
                    <div class="p-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg">
                        ...
                        <p class="text-xl font-bold text-green-600">{{ $daysInMonth }}</p>
                    </div>
                    <div class="p-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg">
                        <p class="text-xs text-gray-600 mb-1">Avg per Day</p>
                        <p class="text-xl font-bold text-green-600">{{ number_format($avgDowntimePerDay, 1) }}m</p>
                    </div>
                    @if($monthDowntimeCount > 0)
                    <div class="p-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg">
                        <p class="text-xs text-gray-600 mb-1">Breakdowns per Day</p>
                        <p class="text-xl font-bold text-green-600">{{ number_format($monthDowntimeCount / $daysInMonth, 1) }}</p>
                    </div>
                    @endif
                </div>
            </div>
        ...
        <div class="info-card rounded-xl shadow-lg p-6 animate-fade-in-up delay-600">
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-xl font-bold bg-gradient-to-r from-red-600 to-pink-600 bg-clip-text text-transparent flex items-center">
                    <svg class="w-6 h-6 mr-2 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    Recent Downtime Events (This Month)
                </h2>
                <a href="{{ route('downtime_erp.index') }}" class="text-sm text-blue-600 hover:text-blue-800 font-semibold transition-all hover:translate-x-1">View all →</a>
            </div>
            <div class="space-y-3 max-h-96 overflow-y-auto pr-2">
                @forelse($recentDowntimeErps as $index => $downtimeErp)
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex-1">
                            <div class="flex items-center gap-3 mb-2">
                                ...
                                <p class="text-lg font-bold text-gray-900">{{ $downtimeErp->idMachine ?? 'N/A' }}</p>
                                <span class="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs font-medium">{{ $downtimeErp->typeMachine ?? 'N/A' }}</span>
                            </div>
                            <div class="flex items-center gap-4 mt-2 flex-wrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                    🏭 {{ $downtimeErp->plant ?? 'N/A' }}
                                </span>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                                    ⚠️ {{ strlen($downtimeErp->problemDowntime ?? 'N/A') > 30 ? substr($downtimeErp->problemDowntime, 0, 30) . '...' : ($downtimeErp->problemDowntime ?? 'N/A') }}
                                </span>
                                @if($downtimeErp->nameMekanik)
                        ...
                        <div class="text-right ml-4">
                            <p class="text-xs text-gray-500 mb-1">{{ $downtimeErp->date ? \Carbon\Carbon::parse($downtimeErp->date)->format('M d, Y') : 'N/A' }}</p>
                            <p class="text-2xl font-bold bg-gradient-to-r from-red-600 to-pink-600 bg-clip-text text-transparent">{{ number_format($downtimeErp->duration ?? 0, 1) }}</p>
