        ...
        <div class="bg-white rounded-lg shadow p-4 sm:p-6 overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($downtimeErp2s as $downtimeErp2)
                    <tr class="{{ $loop->even ? 'bg-gray-50' : 'bg-white' }} hover:bg-gray-100 transition-colors">
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{{ $loop->iteration + ($downtimeErp2s->currentPage() - 1) * $downtimeErp2s->perPage() }}</td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{{ $downtimeErp2->date }}</td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{{ $downtimeErp2->kode_room ?? '-' }}</td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{{ $downtimeErp2->plant }}</td>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-center">
                            <div class="flex flex-row justify-center items-center gap-2">
                                @if(Auth::user()->role !== 'mekanik')
                                    <a href="{{ route('downtime-erp2.edit', ['downtime_erp2' => $downtimeErp2->id, 'page' => request('page', 1)]) }}" class="inline-flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white p-2 rounded shadow transition duration-150 ease-in-out" title="Edit">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                    </a>
                                    <form action="{{ route('downtime-erp2.destroy', $downtimeErp2->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="inline-flex items-center justify-center bg-red-600 hover:bg-red-700 text-white p-2 rounded shadow transition duration-150 ease-in-out" title="Delete" onclick="return confirm('Delete this downtime ERP2?')">
...
<div id="uploadModal" class="hidden fixed inset-0 z-50 overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div class="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <form action="{{ route('downtime-erp2.upload') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-4">
                    ...
                    <p class="mt-2 text-xs text-gray-500">Format Excel: Kolom pertama harus header dengan semua field downtime ERP2. Tambahkan kolom kode_room [opsional] untuk auto-fill plant, process, line, dan roomName dari RoomERP.</p>
                </div>
                <div class="flex items-center gap-3">
