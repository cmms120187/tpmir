...
<div class="max-w-xl mx-auto p-8 bg-white rounded shadow">
    <h1 class="text-2xl font-bold mb-6">Edit Line</h1>
    <form action="{{ route('lines.update', $line->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="mb-4">
            <label for="name" class="block font-semibold mb-2">Nama Line</label>
            <input type="text" name="name" id="name" value="{{ old('name', $line->name) }}" class="w-full border rounded px-3 py-2" required>
            @error('name')
        ...
        <div class="mb-4">
            ...
            <select name="plant_id" id="plant_id" class="w-full border rounded px-3 py-2 @error('plant_id') border-red-500 @enderror" required>
                ...
                @foreach($plants as $plant)
                    <option value="{{ $plant->id }}" {{ old('plant_id', $line->plant_id) == $plant->id ? 'selected' : '' }}>{{ $plant->name }}</option>
                @endforeach
            </select>
            @error('plant_id')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        ...
        <div class="mb-4">
            ...
            <select name="process_id" id="process_id" class="w-full border rounded px-3 py-2 @error('process_id') border-red-500 @enderror" required>
                ...
                @foreach($processes as $process)
                    <option value="{{ $process->id }}" {{ old('process_id', $line->process_id) == $process->id ? 'selected' : '' }}>{{ $process->name }}</option>
                @endforeach
            </select>
            @error('process_id')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
